...
function queryByConditions1({ db, tableName, conditions, callback }) {
    ...
    const orderByStr = conditions.orderBy
        ? `ORDER BY ${conditions.orderBy} ${conditions.orderByDesc ? "DESC" : "ASC"}`
        : "";
...
async function ensureTableColumns({ db, tableName, data, config }) {
    return new Promise((resolve, reject) => {
        db.get(`SELECT sql FROM sqlite_master WHERE type='table' AND name='${tableName}'`, async (err, result) => {
            ...
            const newColumns = getObjectKeys(getColomn).filter((key) => !existingColumns.includes(key));
            if (newColumns.length > 0) {
                const alterPromises = newColumns.map((col) => new Promise((res, rej) => {
                    const type = "TEXT";
                    db.run(`ALTER TABLE ${tableName} ADD COLUMN ${col} ${type}`, (alterErr) => (alterErr ? rej(alterErr) : res("success")));
                }));
...
// 插入/更新数据
export async function upsertData({ db, tableName, data, config, callback, }) {
