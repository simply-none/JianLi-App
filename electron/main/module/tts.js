 ...
 * 系统 TTS 朗读
 */
function speakSystem(text, options = {}) {
    return new Promise((resolve, reject) => {
...
export function initTTS() {
    ...
    // ============ 系统 TTS (say 库) ============
    // 获取系统语音列表
    ipcMain.handle('tts:system:get-voices', async () => {
        try {
    ...
    // 系统 TTS 朗读
    ipcMain.handle('tts:system:speak', async (_event, text, options = {}) => {
        try {
    ...
    // 系统 TTS 停止
    ipcMain.handle('tts:system:stop', async () => {
        try {
            await stopSystem();
            return { success: true };
        }
    ...
    // 检测系统 TTS 是否可用
    ipcMain.handle('tts:system:is-available', async () => {
        try {
    ...
    // 检测所有 TTS 可用性
    ipcMain.handle('tts:is-available', async () => {
