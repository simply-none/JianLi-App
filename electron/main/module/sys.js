...
export async function initSys() {
    ...
    ipcMain.on("get-default-file-path", async (event, { ext }) => {
        return new Promise((resolve) => {
            if (defaultAppWorker) {
    ...
    // 打开文件通过默认程序
    ipcMain.handle("open-file-by-default-app", async (event, { filePath, defaultAppPath }) => {
        return new Promise((resolve) => {
...
$result | Where-Object { $_.Name -and -not $_.IsSystemComponent } | ConvertTo-Json -Depth 5
    ...
    `;
            exec(`powershell -Command "${command}"`, { maxBuffer: 1024 * 10000 }, (error, stdout, stderr) => {
                if (error)
                    return reject(error);
                if (stderr)
