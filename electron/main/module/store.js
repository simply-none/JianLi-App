...
function getDisabledShowKeysData() {
    let storeData = store.store;
    const keys = Object.keys(storeData || {});
    const newStoreData = {};
    keys.forEach((item) => {
...
export async function setPomodoroToWork(force = true) {
    const now = Date.now();
    await upsertData({
        db: myDb.db,
        tableName: tableName,
...
export function initStore() {
    ...
    ipcMain.on("set-store", async (e, key, value) => {
        // if (key === "multi-field") {
    ...
    // store替换
    ipcMain.on("replace-store", (e, value) => {
