...
import { myDb, query, insert, upsert, update, del, ensureTableExists } from './newSql.ts';
 ...
 */
function resolveEncoding(detected) {
    ...
    const candidates = [upper, detected];
 ...
 */
async function ensureBookIdentity(db, filePath, contentHash) {
    if (!contentHash)
        return;
    ...
    for (const table of [
        ...
        await dbRunAsync(db, `UPDATE ${table} SET content_hash = ? WHERE file_path = ? AND (content_hash IS NULL OR content_hash = '')`, [contentHash, filePath]);
    }
}
/**
 ...
 * @returns 成功 resolve void；失败 reject Error（如数据库未初始化）
 */
async function createBookshelfTable() {
    const db = myDb.db;
    if (!db) {
        throw new Error('数据库未初始化');
    }
 ...
 * @returns 成功 resolve void；失败 reject Error（如数据库未初始化）
 */
async function createAnnotationTable() {
    const db = myDb.db;
    if (!db) {
        throw new Error('数据库未初始化');
    }
...
export async function initEbook() {
    ...
    try {
        await ensureTableExists(EBOOK_PROGRESS_TABLE, ['format', 'cfi', 'percent', 'updated_at', 'content_hash'], 'file_path');
        await ensureTableExists(EBOOK_BOOKSHELF_TABLE, ['name', 'format', 'percent', 'last_read_at', 'added_at', 'title', 'author', 'cover', 'content_hash'], 'file_path');
        await ensureTableExists(EBOOK_ANNOTATION_TABLE, ['file_path', 'format', 'anchor', 'text', 'note', 'color', 'type', 'created_at', 'updated_at', 'content_hash'], 'id');
        await ensureTableExists(EBOOK_BOOKMARK_TABLE, ['file_path', 'format', 'cfi', 'label', 'percent', 'created_at', 'content_hash'], 'id');
    }
    catch (err) {
     ...
     */
    ipcMain.handle('ebook:read-txt', async (_event, filePath) => {
        try {
            ...
            log.info(`Read txt file: ${filePath}, size=${size}, encoding=${encoding}`);
     ...
     */
    ipcMain.handle('ebook:read-file-bytes', async (_event, filePath) => {
        try {
            if (!filePath || typeof filePath !== 'string') {
                return { error: '文件路径不能为空' };
            }
     ...
     */
    ipcMain.handle('ebook:compute-file-hash', async (_event, filePath) => {
        try {
            if (!filePath || typeof filePath !== 'string') {
     ...
     */
    ipcMain.handle('ebook:get-progress', async (_event, filePath, contentHash) => {
        try {
            const db = myDb.db;
            if (!db) {
            ...
            if (contentHash) {
                ...
                if (byHash.length) {
                    return { success: true, data: byHash[0] ?? null };
                }
            }
     ...
     */
    ipcMain.handle('ebook:save-progress', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
            ...
            if (contentHash) {
                await dbRunAsync(db, `UPDATE ${EBOOK_PROGRESS_TABLE} SET cfi = ?, percent = ?, updated_at = ?, format = ?, content_hash = ? WHERE content_hash = ? AND file_path != ?`, [data.cfi ?? '', percent, updatedAt, data.format ?? '', contentHash, contentHash, data.filePath]);
            }
            ...
            await upsert({
                ...
                data: {
                    file_path: data.filePath,
                    name: existingRow?.name ?? data.name ?? '',
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:get-bookshelf', async () => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     * @returns 成功返回 { success: true }；失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:add-to-bookshelf', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     * @returns 成功返回 { success: true }；失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:remove-from-bookshelf', async (_event, filePath) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     * @returns 成功返回 { success: true }；失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:save-book-meta', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
            ...
            const upd = await dbRunAsync(db, `UPDATE ${EBOOK_BOOKSHELF_TABLE} SET title = ?, author = ?, cover = ?, content_hash = ? WHERE file_path = ?`, [title, author, cover, contentHash, data.filePath]);
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:get-annotations', async (_event, filePath, contentHash) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
            ...
            if (contentHash) {
                ...
                if (byHash.length) {
                    return { success: true, data: byHash };
                }
            }
            const rows = await query({
                tableName: EBOOK_ANNOTATION_TABLE,
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:add-annotation', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     * @returns 成功返回 { success: true }；失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:update-annotation', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     * @returns 成功返回 { success: true }；失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:remove-annotation', async (_event, id) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:remove-annotations', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:get-annotation-counts', async (_event, filePaths, contentHashes) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     */
    ipcMain.handle('ebook:export-annotations', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
            ...
            let rows;
            const columns = ['id', 'file_path', 'format', 'anchor', 'text', 'note', 'color', 'type', 'created_at', 'updated_at'];
            if (contentHash) {
                rows = await query({
                    tableName: EBOOK_ANNOTATION_TABLE,
                    columns,
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:get-bookmarks', async (_event, filePath, contentHash) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     *          失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:add-bookmark', async (_event, data) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
     ...
     * @returns 成功返回 { success: true }；失败返回 { success: false, error: string }
     */
    ipcMain.handle('ebook:remove-bookmark', async (_event, id) => {
        try {
            const db = myDb.db;
            if (!db) {
                return { success: false, error: '数据库未初始化' };
 ...
 */
function buildAnnotationsMarkdown(title, filePath, records) {
    const lines = [];
    const totalNotes = records.filter((r) => (r.note || '').trim().length > 0).length;
    const totalHighlights = records.length - totalNotes;
    lines.push(`# ${title}`);
    lines.push('');
    lines.push(`> 导出时间：${new Date().toLocaleString('zh-CN')} · 笔记 ${totalNotes} 条 · 划线 ${totalHighlights} 条`);
    lines.push('');
    if (filePath) {
        // 单本：直接输出笔记与划线两个分区
 ...
 * @returns 无返回值
 */
function appendAnnotationSection(lines, records) {
