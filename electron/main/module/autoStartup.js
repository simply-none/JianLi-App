...
function setLoginItemSettings(isStartup) {
    try {
...
function createShortcut(isStartup) {
    if (process.platform !== 'win32') {
        return {
            success: false,
            method: 'shortcut',
...
function createTaskScheduler(isStartup) {
    if (process.platform !== 'win32') {
        return {
            success: false,
...
function cleanupOtherMethods(isStartup) {
    if (!isStartup) {
        createShortcut(false);
        createTaskScheduler(false);
    }
}
export function setAutoStartup(isStartup) {
...
export function checkAutoStartupStatus() {
    if (process.platform === 'win32') {
        const startupFolder = getStartupFolder();
        const shortcutPath = path.join(startupFolder, `${APP_NAME}.lnk`);
        if (fs.existsSync(shortcutPath)) {
