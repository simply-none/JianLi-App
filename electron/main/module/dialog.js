...
function getSelectType(type) {
    ...
    if (type.includes("image")) {
        filters.push({ name: "图片", extensions: ["jpg", "png", "gif", "webp", "svg", "jpeg"] });
    }
    // 如果是视频
    if (type.includes("video")) {
        filters.push({ name: "视频", extensions: ["mkv", "avi", "mp4"] });
    }
    // 如果是音频
    if (type.includes("audio")) {
        filters.push({ name: "音频", extensions: ["mp3", "wav", "aac"] });
    }
    // 如果是可执行文件
    if (type.includes("executable")) {
        filters.push({ name: "可执行文件", extensions: ["exe"] });
    }
    // 如果是压缩文件
    if (type.includes("zip")) {
        filters.push({ name: "压缩文件", extensions: ["zip", "rar", "7z", "tar", "gz", "bz2", "xz"] });
    }
    // 如果是文件
...
export function getFilePath({ openFile, openDirectory, multiSelections, type, }) {
    const properties = [];
    if (openFile) {
...
async function merge() {
    ...
    for (let i = 0; i < fileSliceList.length; i++) {
        ...
        const arr = fs.readFileSync(fileSliceList[i].path +
...
export async function saveFile({ path, name, tempSplit, content, chunkLength, currentChunkIndex, }) {
    try {
        let newPath = "";
        ...
        if (fileSliceList.length == chunkLength) {
            ...
            // 删除缓存文件
            for (let i = 0; i < fileSliceList.length; i++) {
                const oldPath = fileSliceList[i].path +
                    fileSliceList[i].tempSplit +
                    fileSliceList[i].name +
                    "." +
                    fileSliceList[i].currentChunkIndex;
    ...
    try {
        fs.cp(source, target, {
            ...
            filter: (src, dest) => {
                ...
                if (srcPathArr.length == 1 && srcPathArr[0] == "")
                    return true;
                ...
                const isInclude = include && include.some((item) => srcPathArr.includes(item));
                if (isInclude &&
                    ignoreSuffix &&
                ...
                // 过滤掉匹配的文件后缀
                console.log(srcNameSuffix, ignoreSuffix.includes(srcNameSuffix), "srcNameSuffix");
...
// 导出数据到json
export function exportDataToJson(data, path) {
    try {
        // 格式化json数据并导出
...
export function openFileInAssetsManager(filePath) {
...
export function initFile() {
    // 监听获取文件路径
    ipcMain.on("get-file-list", (e, params) => {
    ...
    // 监听文件保存
    ipcMain.on("save-file", async (e, fileSaveObj) => {
        let result = await saveFile(fileSaveObj);
        console.log(result, "result");
        e.returnValue = result;
    });
    // 监听文件夹复制
    ipcMain.on("copy-folder", async (e, copyArgs) => {
...
async function searchAllDrives(startPatha, extensions, ops) {
    // 改成promise
