...
// 生成RSA密钥对
export function generateRSAKeyPair(passphrase = originPassPhrase) {
...
// 解密
export function decrypt(encryptedText, key, passphrase = originPassPhrase) {
...
export function initCrypto() {
    ipcMain.on("encrypt-pwd", (event, arg) => {
        const { text } = arg;
        let RSAKey = null;
        queryByConditions({
            db: myDb.db,
    ...
    ipcMain.on("decrypt-pwd", (event, arg) => {
    ...
    // 比较密码是否相同
    ipcMain.on("compare-pwd", (event, arg) => {
        try {
