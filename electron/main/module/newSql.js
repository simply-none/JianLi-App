...
async function createDBFile() {
    ...
    let cachePath = (store.get("fileCachePath") ||
        userDataPath);
 ...
 */
export async function count(tableName, condition) {
    const db = myDb.db;
    return new Promise((resolve, reject) => {
 ...
 */
export async function execute(sql, params = [], primaryKey = 'id') {
 ...
 * @returns {Promise<void>}
 */
export async function ensureTableExists(tableName, columns, primaryKey = 'id') {
    const db = myDb.db;
    return new Promise((resolve, reject) => {
        db.get("SELECT name FROM sqlite_master WHERE type='table' AND name = ?", [tableName], async (err, row) => {
            if (err) {
                reject(err);
                return;
            }
            ...
            const hasPrimaryKey = await new Promise((resolve, reject) => {
                db.all(`PRAGMA table_info(${tableName})`, [], (err, rows) => {
                    if (err)
                        reject(err);
                    else
                        resolve(rows.some(row => row.pk === 1));
                });
            });
            if (!existingColumns.includes(primaryKey) && !hasPrimaryKey) {
                const alterPromises = [primaryKey].map((col) => new Promise((res) => {
        ...
        db.get(`SELECT sql FROM sqlite_master WHERE type='table' AND name='${tableName}'`, async (err, result) => {
            ...
            if (!result || !result.sql) {
                ...
                if (!existingColumns.includes(pk)) {
                    const alterPromises = [pk].map((col) => new Promise((res) => {
                        const colDef = pk === 'id'
                            ? 'id INTEGER PRIMARY KEY AUTOINCREMENT'
                            : `${pk} TEXT PRIMARY KEY`;
                        db.run(`ALTER TABLE ${tableName} ADD COLUMN ${colDef}`, (alterErr) => {
            ...
            const newExistingColumns = newColumnsPart
                ...
                .filter(Boolean);
            const newColumns = Object.keys(data[0] || {}).filter((key) => !newExistingColumns.includes(key) && key.toLowerCase() !== pk.toLowerCase());
            if (newColumns.length > 0) {
                const alterPromises = newColumns.map((col) => new Promise((res) => {
                    db.run(`ALTER TABLE ${tableName} ADD COLUMN ${col} TEXT`, (alterErr) => {
                        if (alterErr) {
                            const errMsg = alterErr.message;
                            if (!errMsg.includes("duplicate column name")) {
 ...
 * ```
 */
ipcMain.handle("new-sql:query", async (event, options) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:count", async (event, { tableName, condition }) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:insert", async (event, options) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:upsert", async (event, options) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:update", async (event, options) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:delete", async (event, options) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:execute", async (event, { sql, params, primaryKey }) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:explain", async (event, { sql }) => {
    try {
 ...
 * ```
 */
ipcMain.handle("new-sql:transaction", async (event, options) => {
    try {
    ...
    return new Promise((resolve) => {
        db.all("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name", [], (err, rows) => {
            if (err) {
                ...
                resolve({ success: true, data: rows.map((row) => row.name) });
            }
        });
    });
});
 ...
 * ```
 */
ipcMain.handle("new-sql:tableInfo", async (event, { tableName }) => {
    const db = myDb.db;
    return new Promise((resolve) => {
        db.all(`PRAGMA table_info(${tableName})`, [], (err, rows) => {
            if (err) {
