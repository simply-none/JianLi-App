...
export function initApiTest() {
     ...
     */
    ipcMain.on("spider-test", async (e, obj) => {
        try {
            let data = [''];
    ...
    // system-info
    let trafficWorker;
    ipcMain.on('system-info', async (e, userConfig = { type: 'start' }) => {
    ...
    ipcMain.on('system-info-static', async (e, userConfig = { type: 'start' }) => {
...
// 定位元素并操作
async function locatorStep(mystep, page, item) {
