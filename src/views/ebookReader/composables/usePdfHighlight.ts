...
export function usePdfHighlight(ctx: PdfCtx) {
  ...
  /**
   * 点击已有高亮块：弹出操作菜单（转为笔记 / 删除），不再直接删除。
   */
  async function onHighlightClick(annotationId: number, e?: MouseEvent): Promise<void> {
    ...
    // 定位：PDF 无 iframe，点击事件的 clientX/Y 即父视口坐标，直接取为精确点击点
