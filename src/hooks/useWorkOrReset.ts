...
export function useWorkOrRest() {
  const { setForceWorkTimes, setTodayForceWorkTimes, setCurStatus } =
    useGlobalSetting();
  const { forceWorkTimesC, todayForceWorkTimesC, curStatusC } = storeToRefs(
    useGlobalSetting()
  );
  ...
  function initFn() {
    ...
    console.warn(
      currentTime - startWorkTimeC.value,
      workTimeGapC.value * workTimeGapUnitC.value,
      "work"
    );
    console.warn(
      currentTime - closeWorkTimeC.value,
      restTimeGapC.value * restTimeGapUnitC.value,
      "rest"
    );
    ...
    if (!curStatusC.value || curStatusC.value.value === undefined) {
      // 第一次启动，开始工作
      startForceWorkFn({
        gap: workTimeGapC.value * workTimeGapUnitC.value,
        msg: "应用启动中，30s后您将开始进入工作状态。",
        notTimeout: true,
        msgShowTime: firstWaitTime,
        isInitApp: true,
      });
    ...
    if (
      ...
      startForceWorkFn({
        gap:
          workTimeGapC.value * workTimeGapUnitC.value -
          (currentTime - startWorkTimeC.value),
        msg: "应用启动中，由于人为破坏应用运行机制，30s后您将开始进入工作状态。",
        notTimeout: false,
        msgShowTime: firstWaitTime,
      });
    ...
    if (
      ...
      startRestFn({
        gap:
          restTimeGapC.value * restTimeGapUnitC.value -
          (currentTime - closeWorkTimeC.value),
        msg: "应用启动中，由于人为破坏应用运行机制，您应当继续开始进入休息状态",
        notTimeout: true,
      });
    ...
    startForceWorkFn({
      gap: workTimeGapC.value * workTimeGapUnitC.value,
      msg: "应用启动中，上一个应用运行机制已结束，30s后您将开始进入工作状态。",
      notTimeout: false,
      msgShowTime: firstWaitTime,
      isInitApp: true,
    });
  ...
  function startWorkFn(gap?: number, msg?: string) {
    ...
    startForceWorkFn({
      gap: gap || workTimeGapC.value * workTimeGapUnitC.value,
      msg,
    });
  }

  function startForceWorkFn({
    gap,
    msg,
    notTimeout,
    msgShowTime,
    ...
    sysNotify(
      "提示",
      msg || `${(msgShowTime || waitTime) / 1000}秒后开始进入工作状态`,
      ""
    );
    appNotify(
      "提示",
  ...
  function startRestFn({
    ...
    msg?: string;
    notTimeout?: boolean;
    isUpdateCloseTime?: boolean;
  } = {}) {
    if (startTimer.value) clearTimeout(startTimer.value);
    sysNotify("提示", msg || "3秒后开始进入休息状态", "");
    appNotify("提示", msg || "3秒后开始进入休息状态");
    console.warn(
      gap || restTimeGapC.value * restTimeGapUnitC.value,
      "restTimeGapUnitC 休息时间间隔"
    );
    // 判断当前时间是否在11:50-13:30之间
    let isNoonRestTime = false;
    let noonRestTimeGap = 0;
    if (
      moment().isBetween(moment("11:50", "HH:mm"), moment("13:30", "HH:mm"))
    ) {
      console.warn("当前处于11:50-13:30之间", moment().format("HH:mm"));
      isNoonRestTime = true;
      // 计算当前时间距离13:30的时间差(毫秒级)
      noonRestTimeGap = Math.abs(
        moment().diff(moment("13:30", "HH:mm"), "seconds", true) * 1000
      );
  ...
  function startScreenSaverFn({
    ...
    msg,
    notTimeout,
  }: { gap?: number; msg?: string; notTimeout?: boolean } = {}) {
    if (startTimer.value) clearTimeout(startTimer.value);
