...
- 主进程 `electron/main/module/dataAcquisition/`：
  ...
  - `rules.ts`：抽取核心。选择器走 Puppeteer 原生 $/$$（**天然支持 P 选择器** `text/` `xpath//` `aria/` `>>>`）；取值函数为页面内静态函数 `getElValue(el, attr)`；变换管道（trim/replace/number/date/split）在 Node 端执行；`attachCapture` 监听 response 仅捕 JSON、`captureToRecords` 按 dataPath 提取
...
- 渲染端 `src/views/dataAcquisition/`：
  ...
  - `components/`：`sidebar/TaskList`、`config/TaskConfigPanel`（**按浏览器使用流程分步**：①打开网页（URL/数据源/页面等待）→ ②模拟浏览操作（交互步骤）→ ③提取结果（记录容器/字段规则/接口捕获，**作用于②执行完后的最终页面**）→ ④分页；含 RuleEditor/ActionPanel/PaginationPanel/AntiCrawlPanel + JSON 高级编辑 + ConfigGuide 可折叠说明）、`run/RunPanel`、`result/ResultView`（动态列表格+JSON 双视图+分页）、`history/HistoryPanel`（查看弹窗/重跑）
...
| `net-request:save-file` | 渲染→主 | CSV 导出复用（text 写盘，返回 `{ success, path }`） |

## 持久化
- SQLite `scraper_tasks`（name 唯一索引 uq_scraper_task_name，按 name 幂等保存）、`scraper_history`（保留最近 100 条，data 超 2MB 截断为 []）
